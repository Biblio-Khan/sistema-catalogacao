...
with st.form("form_cadastro"):
   ...
   # --- Seção: Banca Examinadora ---
    ...
    for i in range(num_orientadores):
        ...
        with col1:
            nome = st.text_input(f"Nome do orientador {i+1}", key=f"nome_o_{i}")
        with col2:
            titulacao = st.selectbox(f"Título O", titulos_opcoes, key=f"tit_o_{i}")
        if nome:
    ...
    for i in range(num_coorientadores):
        col1, col2 = st.columns([3, 1])
        with col1:
            nome = st.text_input(f"Nome do coorientador {i+1}", key=f"nome_c_{i}")
        with col2:
            titulacao = st.selectbox(f"Título C", titulos_opcoes, key=f"tit_c_{i}")
        if nome:
            lista_coorientadores.append(f"{titulacao} {nome}")
    # -----------------------------------------------
